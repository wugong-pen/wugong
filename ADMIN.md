...
- 後台「配送國家／運費」可新增地區、設定每筆固定新台幣運費、啟用或停用，內部成本備註不對外顯示。
- 台灣沿用既有 NT$0；日本、韓國、美國、新加坡、香港先建立待確認項目，未填費率並啟用前不能建立新訂單。空白不是免運；填 0 才表示免運。其他地區預設不開放。
- 美國歷史包裹 24.3 × 15.2 × 10.25 cm、0.7 kg、NT$1,544 僅作成本參考，尚非對客收費。物流、保險、關稅及支付成本仍待確認。其他國家費率尚未決定。
- 運費為每筆訂單固定費率，非按重量自動計算；開放前須確認多件商品及較大包裹也適用。海外墨水商品及墨水贈品均禁止。
- 運費由伺服器於商品折扣後另計。每筆新訂單留存運費快照，修改設定不會重算舊訂單。原有未付款訂單保留原付款條件；停用地區會阻止新下單。
- 配送設定與會員註冊國家分開。付款選項還須具備可用的支付設定，目前仍僅測試金流。
- 配送設定修改保留操作紀錄，過期編輯會拒絕，結帳時費率變更會要求重新確認。

## 店主確認運費（2026-09-23）

日本 NT$500、韓國 NT$700、新加坡 NT$750、香港 NT$500、美國 NT$1,600，皆為每筆訂單固定運費，啟用配送。海外運費不含進口關稅，關稅由收件人另行負擔。

本次以一次性資料更新套用至既有資料庫，並留下操作紀錄；之後在後台修改費率或停用地區，不會被重新載入覆寫。既有訂單金額維持原快照。上方待確認文字為先前設定紀錄，本節為目前已確認規則。
...
## Administrator order email notifications — 2026-09-25
New successful orders atomically enqueue a plain-text email snapshot addressed only to wugong.pen@gmail.com. Uses the existing Resend secret, MAIL_FROM and MAIL_ORIGIN. Existing orders are not backfilled. Order confirmation schedules delivery; the existing five-minute cron retries with a database lease and the same Resend idempotency key/payload. Messages stop for manual attention after ten attempts or 23 hours from first attempt to avoid retrying beyond the provider's 24-hour deduplication window. Missing configuration retains the queue without attempting delivery.
Order management and detail pages expose restricted notification status, plus an administrator-only rate-limited synthetic test email button. Provider acceptance is not inbox delivery. No payment status or inventory is changed by notification failures.
