...
const previousPen="SELECT 1 FROM member_orders mo JOIN orders o ON o.order_number=mo.order_number, json_each(CASE WHEN json_valid(o.items) THEN o.items ELSE '[]' END) i WHERE mo.member_id=? AND o.order_number!=? AND o.status NOT IN ('cancelled','expired','payment_failed') AND (json_extract(i.value,'$.category')='pen' OR EXISTS(SELECT 1 FROM products p WHERE p.sku=json_extract(i.value,'$.id') AND p.category='pen'))";
export async function firstGiftQuote(env,items,member,country){
 if(!country||!items.some(i=>i.category==='pen'))return null;
 const settings=await firstSettings(env);await env.DB.prepare("DELETE FROM first_purchase_claims WHERE order_number IN (SELECT o.order_number FROM orders o LEFT JOIN checkout_reservations r ON r.order_number=o.order_number WHERE r.state='released' OR o.status IN ('cancelled','expired','payment_failed'))").run();
 if(await env.DB.prepare(previousPen).bind(member,'').first()||await env.DB.prepare('SELECT 1 FROM first_purchase_claims WHERE member_id=?').bind(member).first())return null;
...
export function firstGiftStatements(env,gift,member,order){if(!gift)return[];return[
 ...
 env.DB.prepare("INSERT INTO catalog_checks(ok) SELECT CASE WHEN (?=0 AND NOT EXISTS(SELECT 1 FROM first_purchase_settings WHERE region=?)) OR EXISTS(SELECT 1 FROM first_purchase_settings WHERE region=? AND version=? AND active=1) THEN 1 ELSE 0 END").bind(gift.version,gift.region,gift.region,gift.version),
