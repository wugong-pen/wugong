...
export async function discountQuote(env,q,code,member){
 ...
 code=text(code,40).toUpperCase();const c=await env.DB.prepare('SELECT * FROM coupons WHERE code=?').bind(code).first(),now=new Date().toISOString();
 if(!c||!c.active||c.starts>now||c.ends<=now)fail(409,'優惠券不存在、未開始或已到期');
 if(await env.DB.prepare('SELECT 1 FROM coupon_claims WHERE code=? AND member_id=?').bind(code,member).first())fail(409,'此優惠券已使用或有未完成訂單保留中');
 const count=await env.DB.prepare('SELECT count(*) AS n FROM coupon_claims WHERE code=?').bind(code).first();if(count.n>=c.quota)fail(409,'優惠券名額已用完');
 const eligible=q.items.filter(i=>c.scope==='all'||(c.scope==='family'?i.family===c.target:i.category===c.target)).reduce((sum,i)=>sum+i.price*i.quantity,0);
 if(!eligible||eligible<c.minimum)fail(409,'適用商品金額未達優惠券最低消費');
 q.discount=Math.min(c.maximum,eligible,c.kind==='fixed'?c.amount:Math.floor(eligible*c.amount/100));
 if(q.discount<1||q.total-q.discount<1)fail(400,'折抵後訂單金額須至少為 NT$1');
...
export function couponStatements(env,q,member,order){if(!q.coupon)return [];const c=q.coupon,now=new Date().toISOString();return [
 env.DB.prepare('INSERT INTO catalog_checks(ok) SELECT CASE WHEN EXISTS(SELECT 1 FROM coupons WHERE code=? AND version=? AND active=1 AND starts<=? AND ends>?) AND (SELECT count(*) FROM coupon_claims WHERE code=?)<? AND NOT EXISTS(SELECT 1 FROM coupon_claims WHERE code=? AND member_id=?) THEN 1 ELSE 0 END').bind(c.code,c.version,now,now,c.code,c.quota,c.code,member),
 env.DB.prepare('INSERT INTO coupon_claims VALUES (?,?,?,?)').bind(c.code,member,order,q.discount),env.DB.prepare('INSERT INTO order_discounts VALUES (?,?,?,?)').bind(order,c.code,q.subtotal,q.discount)];}
export async function manageModules(request,env,url,m,body){
 ...
 if(path==='/api/admin/coupons'&&method==='GET')return{coupons:(await env.DB.prepare('SELECT c.*,(SELECT count(*) FROM coupon_claims r WHERE r.code=c.code) AS used FROM coupons c ORDER BY code LIMIT 500').all()).results};
 if(path==='/api/admin/coupon'&&method==='GET'){const code=text(url.searchParams.get('code')||'',40);return{claims:(await env.DB.prepare('SELECT r.member_id,r.order_number,r.discount,o.status FROM coupon_claims r JOIN orders o ON o.order_number=r.order_number WHERE r.code=? ORDER BY o.created_at DESC LIMIT 100').bind(code).all()).results};}
 if(path==='/api/admin/coupon'&&method==='POST'){
  ...
  const kind=text(d.kind,20),scope=text(d.scope,20),target=text(d.target||'',100);if(!['fixed','percent'].includes(kind)||!['all','family','category'].includes(scope))fail(400,'優惠券設定不正確');
  if(scope==='category'&&!['pen','ink','craft'].includes(target))fail(400,'請選擇適用分類');if(scope==='family'&&!await env.DB.prepare('SELECT family FROM product_families WHERE family=?').bind(target).first())fail(400,'請選擇適用商品');
  const amount=num(d.amount,1,kind==='percent'?100:10000000),minimum=num(d.minimum,0,10000000),maximum=num(d.maximum,1,10000000),quota=num(d.quota,1,1000000),active=num(d.active,0,1);
  ...
  const old=await env.DB.prepare('SELECT * FROM coupons WHERE code=?').bind(code).first();if((old?.version||0)!==d.version)fail(409,'優惠券已更新，請重新載入');
  const values=[kind,amount,minimum,maximum,scope,target,starts,ends,quota,active];const stmt=old?env.DB.prepare('UPDATE coupons SET kind=?,amount=?,minimum=?,maximum=?,scope=?,target=?,starts=?,ends=?,quota=?,active=?,version=version+1 WHERE code=? AND version=?').bind(...values,code,d.version):env.DB.prepare('INSERT INTO coupons(kind,amount,minimum,maximum,scope,target,starts,ends,quota,active,code) VALUES (?,?,?,?,?,?,?,?,?,?,?)').bind(...values,code);
