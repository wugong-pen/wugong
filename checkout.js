...
let couponCode='',firstCouponCode='';
...
$('country').addEventListener('change',async()=>{if(busy||pendingOrder)return;try{const r=await fetch('/api/checkout/quote',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({items:cart(),coupon:couponCode,firstCoupon:firstCouponCode,country:$('country').value})}),d=await r.json();if(!r.ok)throw Error(d.error);showFirstGift(d.firstGift);await updateMethods();}catch(e){$('firstGiftSummary').textContent='請確認收件國家與優惠券後重新套用。';show(e.message);}});
