...
export async function manageHomepage(request,env,m,body){
 ...
 for(const path of [config.image,config.backgroundImage])if(path.startsWith('/media/')&&!await env.DB.prepare('SELECT id FROM product_images WHERE id=?').bind(path.slice(7)).first())throw Object.assign(new Error('照片不存在，請重新上傳'),{status:400});
