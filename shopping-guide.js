...
try{
 ...
 for(const region of regions){const row=document.createElement('tr'),name=document.createElement('th'),fee=document.createElement('td');name.scope='row';name.textContent=region.country==='TW'?'台灣':region.country==='KR'?'韓國':names.of(region.country);fee.textContent='NT$'+region.fee.toLocaleString('en-US');row.append(name,fee);body.append(row);}
